...
export default function PatternLibrary({ supaUrl, supaKey }) {
  ...
  const runAISearch = async () => {
    ...
    try {
      ...
      const prompt = "You are a forex chart-pattern librarian with full access to the trader's own saved library of classified cases. Each case has an index i. Answer the trader's question about their library: which cases fit, what they share, what the data shows. WRITE THE ANSWER IN THE SAME LANGUAGE THE TRADER USED IN THEIR QUESTION. Reply ONLY with valid JSON (keys in English), no markdown, no preamble, in this exact shape: {\"answer\": \"2-5 sentence plain-text answer, in the trader's language\", \"matches\": [i, i, ...]} where matches lists the indices of the cases relevant to the question (empty array if none fit). LIBRARY: " + JSON.stringify(lib) + " QUESTION: " + q;
