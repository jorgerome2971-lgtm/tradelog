...
export default function App() {
  ...
  const dbGet = async (table) => { const r = await fetch(`${SUPA_URL}/rest/v1/${table}?order=created_at.asc`, {headers: H}); return r.json(); };
  const dbUpsert = async (table, rows) => { const r = await fetch(`${SUPA_URL}/rest/v1/${table}`, {method:"POST", headers:{...H, Prefer:"resolution=merge-duplicates,return=representation"}, body:JSON.stringify(rows)}); return r.json(); };
  const dbDelete = async (table, id) => { await fetch(`${SUPA_URL}/rest/v1/${table}?id=eq.${id}`, {method:"DELETE", headers:H}); };
  ...
  const saveAccounts = async (data) => {
    const prev = accounts; setAccounts(data); setSaveStatus("Saving...");
    try {
  ...
  const saveTrades = async (data) => {
    const prev = trades; setTrades(data); setSaveStatus("Saving...");
    try {
  ...
  const savePatterns = async (data) => {
    const prev = patterns; setPatterns(data); setSaveStatus("Saving...");
    try {
  ...
  const savePairs = async (data) => {
    const prev = pairs; setPairs(data); setSaveStatus("Saving...");
    try {
      const deleted = prev.filter(p => !data.find(d => d.id === p.id));
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "IBM Plex Mono, monospace" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 26px", borderBottom: `1px solid ${C.border}`, background: C.panel, flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            {tab === "dashboard" && <><Btn ghost onClick={() => setModal({ type: "pattern" })} color={C.accent}>+ Pattern</Btn><Btn onClick={() => setModal({ type: "trade" })}>+ Trade</Btn></>}
          </div>
        </div>
      ...
      {modal?.type === "account" && <AccountModal account={modal.id ? accounts.find(a => a.id === modal.id) : null} onClose={close} onSave={acc => { saveAccounts(modal.id ? accounts.map(a => a.id === modal.id ? acc : a) : [...accounts, acc]); close(); }} onDelete={id => { saveAccounts(accounts.filter(a => a.id !== id)); close(); }} />}
      {modal?.type === "trade" && <TradeModal trade={modal.id ? trades.find(t => t.id === modal.id) : null} accounts={accounts} patterns={patterns} pairs={pairs} allTrades={trades} onClose={close} onSave={t => { if (modal.id) { saveTrades(trades.map(x => x.id === modal.id ? t : x)); } else { const newTrades = Array.isArray(t) ? t : [t]; saveTrades([...trades, ...newTrades]); } close(); }} onDelete={id => { saveTrades(trades.filter(t => t.id !== id)); close(); }} />}
      {modal?.type === "pattern" && <PatternModal pattern={modal.id ? patterns.find(p => p.id === modal.id) : null} onClose={close} onSave={p => { savePatterns(modal.id ? patterns.map(x => x.id === modal.id ? p : x) : [...patterns, p]); close(); }} onDelete={id => { savePatterns(patterns.filter(p => p.id !== id)); close(); }} />}
      {modal?.type === "pair" && <PairModal pair={modal.id ? pairs.find(p => p.id === modal.id) : null} onClose={close} onSave={p => { savePairs(modal.id ? pairs.map(x => x.id === modal.id ? p : x) : [...pairs, p]); close(); }} onDelete={id => { savePairs(pairs.filter(p => p.id !== id)); close(); }} />}
      {modal?.type === "view-trade" && <TradeViewModal trade={trades.find(t => t.id === modal.id)} acctName={acctName} patName={patName} allTrades={trades} patterns={patterns} onClose={close} onEdit={() => setModal({ type: "trade", id: modal.id })} />}
      {modal?.type === "view-pattern" && <PatternViewModal pattern={patterns.find(p => p.id === modal.id)} trades={trades} onClose={close} onEdit={() => setModal({ type: "pattern", id: modal.id })} />}
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(6,1fr)", gap: 10, marginBottom: 22 }}>
        {[{ label: "TOTAL P&L", val: fmt(stats.totalPnl), color: stats.totalPnl >= 0 ? C.green : C.red },{ label: "TRADES", val: stats.total },{ label: "WINNERS", val: stats.wins, color: C.green },{ label: "LOSERS", val: stats.losses, color: C.red },{ label: "WIN RATE", val: stats.winrate },{ label: "R:R", val: stats.rr !== "—" ? stats.rr + ":1" : "—" }].map(s => (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div>
          ...
          {!trades.length ? <Empty text="No trades yet" /> : [...trades].reverse().slice(0, 5).map(t => <TradeRow key={t.id} t={t} acctName={acctName} patName={patName} onClick={() => onViewTrade(t.id)} />)}
        </div>
        <div>
          <SLabel>TOP PATTERNS</SLabel>
          {!topPatterns.length ? <Empty text="No pattern data yet" /> : topPatterns.map(([id, d]) => (
            <div key={id} style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 8, padding: "12px 14px", marginBottom: 6, display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 700 }}>{patName(id)}</div><div style={{ fontSize: 10, color: C.dim, marginTop: 2 }}>{d.wins}/{d.total} wins</div></div>
  ...
  return (
    <div onClick={onClick} style={{ display: "flex", alignItems: "center", gap: 10, background: C.panel, border: `1px solid ${C.border}`, borderRadius: 8, padding: "11px 14px", marginBottom: 6, cursor: "pointer" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 10, color: C.dim, marginTop: 3 }}>{[t.date, t.time, t.day, full && t.session, acctName(t.accountId)].filter(Boolean).join(" · ")}</div>
      </div>
...
TRADES: ${JSON.stringify(trades.map(t => ({ date: t.date, day: t.day, time: t.time, pair: t.pair, type: t.type, pattern: patName(t.patternId), session: t.session, emotion: t.emotion || "—", result: t.result, pnl: t.pnl, riskPct: t.riskPct })))}
JSON: {"overallAssessment":"...","oneThingToFocusOn":"...","strengths":["..."],"weaknesses":["..."],"psychologyInsights":"...","bestDayTime":"...","bestPair":"...","actionPlan":["...","...","...","..."]}`;
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json", "x-api-key": "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", "anthropic-version": "2023-06-01", "anthropic-dangerous-direct-browser-access": "true" }, body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 1000, messages: [{ role: "user", content: prompt }] }) });
  ...
  return (
    <div style={{ maxWidth: 720 }}>
      ...
      {!insights.length ? <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 8, padding: "14px 16px", marginBottom: 20, color: C.dim, fontSize: 13 }}>Log at least 3 trades to generate insights.</div> : insights.map((ins, i) => (
      ...
      {aiLoading && (<div style={{ background: C.panel, border: `1px solid ${C.accent}33`, borderRadius: 10, padding: "32px 20px", textAlign: "center" }}><div style={{ width: 36, height: 36, border: `3px solid ${C.border}`, borderTopColor: C.accent, borderRadius: "50%", animation: "spin .9s linear infinite", margin: "0 auto 14px" }} /><div style={{ color: C.accent, fontSize: 12, letterSpacing: 2 }}>CLAUDE IS ANALYZING YOUR DATA...</div></div>)}
      {aiResult && !aiLoading && (aiResult.error ? <div style={{ color: C.red, fontSize: 13, padding: 16 }}>Could not generate analysis. Try again.</div> : <div>
        <SLabel>⊕ AI DEEP ANALYSIS</SLabel>
        <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: "15px 17px", marginBottom: 12 }}><div style={{ fontSize: 9, color: C.muted, letterSpacing: 2, marginBottom: 8 }}>OVERALL ASSESSMENT</div><div style={{ fontSize: 13, color: C.text, lineHeight: 1.7 }}>{aiResult.overallAssessment}</div></div>
        <div style={{ background: `${C.accent}08`, border: `1px solid ${C.accent}55`, borderRadius: 10, padding: "15px 17px", marginBottom: 12 }}><div style={{ fontSize: 9, color: C.accent, letterSpacing: 2, marginBottom: 8 }}>#1 THING TO FOCUS ON</div><div style={{ fontSize: 15, fontWeight: 700, color: C.accent }}>{aiResult.oneThingToFocusOn}</div></div>
        ...
        <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: "15px 17px", marginBottom: 12 }}><div style={{ fontSize: 9, color: C.gold, letterSpacing: 2, marginBottom: 8 }}>🧠 PSYCHOLOGY</div><div style={{ fontSize: 13, color: C.text, lineHeight: 1.7 }}>{aiResult.psychologyInsights}</div></div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
          {[["BEST DAY/TIME", aiResult.bestDayTime], ["BEST PAIR", aiResult.bestPair]].map(([l, v]) => (<div key={l} style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: "15px 17px" }}><div style={{ fontSize: 9, color: C.muted, letterSpacing: 2, marginBottom: 8 }}>{l}</div><div style={{ fontSize: 13, color: C.text, lineHeight: 1.7 }}>{v}</div></div>))}
        </div>
        <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: "15px 17px" }}><div style={{ fontSize: 9, color: C.accent, letterSpacing: 2, marginBottom: 10 }}>📋 ACTION PLAN</div>{(aiResult.actionPlan || []).map((a, i) => <div key={i} style={{ fontSize: 12, color: C.text, lineHeight: 1.7, padding: "5px 0", borderBottom: `1px solid ${C.border}66`, display: "flex", gap: 10 }}><span style={{ color: C.accent, fontWeight: 700, minWidth: 18 }}>{i + 1}.</span>{a}</div>)}</div>
      </div>)}
    </div>
  );
}
...
function PairsLog({ pairs, onEdit }) {
  if (!pairs.length) return (<div><Empty text="No pairs yet. Add the forex pairs you trade!" /><div style={{ textAlign: "center", marginTop: 16, fontSize: 11, color: C.muted, lineHeight: 1.8 }}>Examples: EUR/USD, GBP/USD, XAU/USD, US30, NAS100...</div></div>);
  return (<div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 12 }}>{pairs.map(p => (<div key={p.id} onClick={() => onEdit(p.id)} style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: "16px 18px", cursor: "pointer" }}><div style={{ fontFamily: "Bebas Neue, sans-serif", fontSize: 22, letterSpacing: 3, color: C.accent, marginBottom: 6 }}>{p.symbol}</div>{p.description && <div style={{ fontSize: 11, color: C.dim, lineHeight: 1.6 }}>{p.description}</div>}<div style={{ fontSize: 9, color: C.muted, marginTop: 10, letterSpacing: 1 }}>✎ click to edit</div></div>))}</div>);
}
...
function PairModal({ pair, onClose, onSave, onDelete }) {
  ...
  const save = () => { if (!symbol.trim()) return alert("Symbol required"); onSave({ id: p.id || uid(), symbol: symbol.toUpperCase().trim(), description }); };
  return (
    <Modal title={p.id ? "EDIT PAIR" : "NEW PAIR"} onClose={onClose}>
      ...
      <div style={{ marginBottom: 13 }}><div style={{ fontSize: 9, color: C.dim, letterSpacing: 2, marginBottom: 5 }}>NOTES (optional)</div><textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="e.g. Best during London session..." rows={3} style={{ width: "100%", background: C.bg, border: `1px solid ${C.border}`, color: C.text, padding: "9px 12px", borderRadius: 6, fontSize: 12, fontFamily: "IBM Plex Mono, monospace", resize: "vertical" }} /></div>
      <div style={{ display: "flex", gap: 10 }}><Btn onClick={save} full>Save Pair</Btn>{p.id && <Btn danger onClick={() => { if (confirm("Delete?")) onDelete(p.id); }}>Delete</Btn>}</div>
    </Modal>
  );
}
...
function AccountLog({ accounts, trades, onEdit }) {
  ...
  return (<div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: 14 }}>{accounts.map(acc => { const at = trades.filter(t => t.accountId === acc.id); const apnl = at.reduce((a, t) => a + (t.pnl || 0), 0); return (<div key={acc.id} onClick={() => onEdit(acc.id)} style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: 18, cursor: "pointer" }}><div style={{ fontFamily: "Bebas Neue, sans-serif", fontSize: 18, letterSpacing: 2, marginBottom: 3 }}>{acc.name}</div><div style={{ fontSize: 10, color: C.dim, marginBottom: 12 }}>{acc.broker}{acc.size ? ` · $${Number(acc.size).toLocaleString()}` : ""}</div><div style={{ fontFamily: "Bebas Neue, sans-serif", fontSize: 24, color: apnl >= 0 ? C.green : C.red, marginBottom: 10 }}>{fmt(apnl)}</div><div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: C.muted }}><span>{at.length} trades</span><span>{acc.currency || "USD"}</span></div><div style={{ fontSize: 9, color: C.muted, marginTop: 8, letterSpacing: 1 }}>✎ click to edit</div></div>); })}</div>);
}
...
function AccountModal({ account, onClose, onSave, onDelete }) {
  ...
  return (
    <Modal title={a.id ? "EDIT ACCOUNT" : "NEW ACCOUNT"} onClose={onClose}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}><Inp label="BROKER / FIRM" value={broker} onChange={setBroker} placeholder="FTMO, TopStep..." /><Inp label="ACCOUNT SIZE ($)" value={size} onChange={setSize} type="number" placeholder="10000" /></div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}><Sel label="CURRENCY" value={currency} onChange={setCurrency} options={CURRENCIES} /><Inp label="MAX DAILY LOSS %" value={maxDaily} onChange={setMaxDaily} type="number" placeholder="5" /></div>
      <Inp label="MAX DRAWDOWN %" value={maxDrawdown} onChange={setMaxDrawdown} type="number" placeholder="10" />
      <div style={{ display: "flex", gap: 10, marginTop: 4 }}><Btn onClick={save} full>Save Account</Btn>{a.id && <Btn danger onClick={() => { if (confirm("Delete this account?")) onDelete(a.id); }}>Delete</Btn>}</div>
    </Modal>
  );
}
...
function TradeModal({ trade, accounts, patterns, pairs, allTrades, onClose, onSave, onDelete }) {
  const t = trade || {};
  const [selectedAccountIds, setSelectedAccountIds] = useState(t.accountId ? [t.accountId] : (accounts[0] ? [accounts[0].id] : []));
  const toggleAccount = (id) => setSelectedAccountIds(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);
  ...
  const [pair, setPair] = useState(t.pair || "");
  ...
  const handleResultChange = async (newResult) => {
    ...
    if (newResult === "win" || newResult === "loss" || newResult === "breakeven") {
      setMiniCoachLoading(true); setMiniCoach(null);
      const history = (allTrades || []).slice(-15).map(tr => ({ pair: tr.pair, result: tr.result, pnl: tr.pnl, emotion: tr.emotion, pattern: patName(tr.patternId), entryNotes: tr.entryNotes || "", exitNotes: tr.exitNotes || "", mistakes: tr.mistakes || "" }));
      const patRules = patterns.map(p => ({ name: p.name, rules: p.rules }));
      const prompt = `You are a brief forex trading coach. Trade closed as "${newResult}". Give punchy coaching. Respond ONLY with valid JSON:
...
RULES: ${JSON.stringify(patRules)}
HISTORY: ${JSON.stringify(history)}
JSON: {"message":"2-3 sentences coaching","emoji":"one emoji","tone":"positive|warning|neutral"}`;
      try {
  ...
  const save = () => {
    ...
    if (t.id) { onSave({ ...base, id: t.id, accountId: selectedAccountIds[0] }); }
    else { onSave(selectedAccountIds.map(accId => ({ ...base, id: uid(), accountId: accId }))); }
  };
    ...
    <Modal title={t.id ? "EDIT TRADE" : "NEW TRADE"} onClose={onClose}>
      <div style={{ marginBottom: 13 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {accounts.map(a => (<div key={a.id} onClick={() => !t.id && toggleAccount(a.id)} style={{ padding: "7px 14px", borderRadius: 6, border: `1px solid ${selectedAccountIds.includes(a.id) ? C.accent : C.border}`, background: selectedAccountIds.includes(a.id) ? `${C.accent}15` : C.bg, color: selectedAccountIds.includes(a.id) ? C.accent : C.dim, fontSize: 12, cursor: t.id ? "default" : "pointer", fontFamily: "IBM Plex Mono, monospace" }}>{selectedAccountIds.includes(a.id) ? "✓ " : ""}{a.name}</div>))}
        </div>
        {!t.id && selectedAccountIds.length > 1 && <div style={{ fontSize: 10, color: C.gold, marginTop: 6 }}>⚡ Will create {selectedAccountIds.length} trades — one per account</div>}
      </div>
      <div style={{ marginBottom: 13 }}><div style={{ fontSize: 9, color: C.dim, letterSpacing: 2, marginBottom: 5 }}>DATE</div><input type="date" value={date} onChange={e => setDate(e.target.value)} style={{ width: "100%", background: C.bg, border: `1px solid ${C.border}`, color: C.text, padding: "9px 12px", borderRadius: 6, fontSize: 12, fontFamily: "IBM Plex Mono, monospace" }} /></div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}><Sel label="DAY" value={day} onChange={setDay} options={DAYS} /><Inp label="TIME" value={time} onChange={setTime} type="time" /><Sel label="SESSION" value={session} onChange={setSession} options={SESSIONS} /></div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div style={{ marginBottom: 13 }}><div style={{ fontSize: 9, color: C.dim, letterSpacing: 2, marginBottom: 5 }}>PAIR</div><select value={pair} onChange={e => setPair(e.target.value)} style={{ width: "100%", background: C.bg, border: `1px solid ${C.border}`, color: C.text, padding: "9px 12px", borderRadius: 6, fontSize: 12, fontFamily: "IBM Plex Mono, monospace" }}><option value="">Select pair...</option>{pairs && pairs.length > 0 ? pairs.map(p => <option key={p.id} value={p.symbol}>{p.symbol}</option>) : PAIRS.map(p => <option key={p} value={p}>{p}</option>)}</select></div>
        <Sel label="TYPE" value={type} onChange={setType} options={[{ value: "buy", label: "BUY (Long)" }, { value: "sell", label: "SELL (Short)" }]} placeholder="" />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}><Sel label="PATTERN USED" value={patternId} onChange={setPatternId} options={patterns.map(p => ({ value: p.id, label: p.name }))} placeholder="— No pattern —" /><Sel label="TIMEFRAME" value={timeframe} onChange={setTimeframe} options={TIMEFRAMES} /></div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}><Inp label="RISK $ AMOUNT" value={riskAmount} onChange={setRiskAmount} type="number" placeholder="0.00" /><Sel label="EMOTION AT ENTRY" value={emotion} onChange={setEmotion} options={EMOTIONS} /></div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}><Inp label="TRADINGVIEW ENTRY LINK" value={entryLink} onChange={setEntryLink} placeholder="https://..." /><Inp label="TRADINGVIEW EXIT LINK" value={exitLink} onChange={setExitLink} placeholder="https://..." /></div>
      <div style={{ background: `${C.accent}08`, border: `1px solid ${C.accent}22`, borderRadius: 8, padding: "14px", marginBottom: 13 }}>
        ...
        <TA label="ENTRY NOTES — What do you see? Why are you entering?" value={entryNotes} onChange={setEntryNotes} placeholder={"What pattern did you see?\nWhy does this setup make sense?\nWhat are you expecting to happen?"} rows={3} color={C.accent} />
        <TA label="EXIT NOTES — What happened? What did you observe?" value={exitNotes} onChange={setExitNotes} placeholder={"What happened after entry?\nDid price do what you expected?\nWhen/why did you exit?"} rows={3} color={C.green} />
        <TA label="MISTAKES / ERRORS — What went wrong? (write 'none' if no errors)" value={mistakes} onChange={setMistakes} placeholder={"Did you enter too early?\nDid you move your stop?\n→ Write 'none' if everything was correct."} rows={2} color={C.red} />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
        <div style={{ marginBottom: 13 }}><div style={{ fontSize: 9, color: C.dim, letterSpacing: 2, marginBottom: 5 }}>RESULT</div><select value={result} onChange={e => handleResultChange(e.target.value)} style={{ width: "100%", background: C.bg, border: `1px solid ${C.border}`, color: C.text, padding: "9px 12px", borderRadius: 6, fontSize: 12, fontFamily: "IBM Plex Mono, monospace" }}>{["pending","win","loss","breakeven"].map(r => <option key={r} value={r}>{r.charAt(0).toUpperCase()+r.slice(1)}</option>)}</select></div>
      ...
      {miniCoachLoading && (<div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 14px", background: C.bg, border: `1px solid ${C.accent}22`, borderRadius: 8, marginBottom: 13 }}><div style={{ width: 16, height: 16, border: `2px solid ${C.border}`, borderTopColor: C.accent, borderRadius: "50%", animation: "spin .7s linear infinite", flexShrink: 0 }} /><div style={{ fontSize: 11, color: C.dim }}>AI coach reviewing your trade...</div></div>)}
      {miniCoach && !miniCoachLoading && (<div style={{ padding: "12px 14px", background: miniCoach.tone === "positive" ? `${C.green}0a` : miniCoach.tone === "warning" ? `${C.red}0a` : C.bg, border: `1px solid ${miniCoach.tone === "positive" ? C.green : miniCoach.tone === "warning" ? C.red : C.border}33`, borderRadius: 8, marginBottom: 13 }}><div style={{ fontSize: 9, color: miniCoach.tone === "positive" ? C.green : miniCoach.tone === "warning" ? C.red : C.muted, letterSpacing: 2, marginBottom: 6 }}>⊕ AI MINI COACH</div><div style={{ fontSize: 12, lineHeight: 1.7, color: C.text }}>{miniCoach.emoji} {miniCoach.message}</div></div>)}
      <div style={{ display: "flex", gap: 10 }}><Btn onClick={save} full>Save Trade</Btn>{t.id && <Btn danger onClick={() => { if (confirm("Delete?")) onDelete(t.id); }}>Delete</Btn>}</div>
    </Modal>
  );
}
...
function PatternModal({ pattern, onClose, onSave, onDelete }) {
  ...
  return (
    <Modal title={p.id ? "EDIT PATTERN" : "NEW PATTERN"} onClose={onClose}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}><Sel label="TIMEFRAME" value={timeframe} onChange={setTimeframe} options={TIMEFRAMES} /><Sel label="BEST SESSION" value={session} onChange={setSession} options={SESSIONS} /></div>
      <Inp label="PAIRS (comma separated)" value={pairs} onChange={setPairs} placeholder="EUR/USD, GBP/USD..." />
      <div style={{ marginBottom: 13 }}><div style={{ fontSize: 9, color: C.dim, letterSpacing: 2, marginBottom: 5 }}>DESCRIPTION</div><textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="What conditions must be met?" rows={3} style={{ width: "100%", background: C.bg, border: `1px solid ${C.border}`, color: C.text, padding: "9px 12px", borderRadius: 6, fontSize: 12, fontFamily: "IBM Plex Mono, monospace", resize: "vertical" }} /></div>
      <div style={{ marginBottom: 13 }}><div style={{ fontSize: 9, color: C.dim, letterSpacing: 2, marginBottom: 5 }}>ENTRY RULES</div><textarea value={rules} onChange={e => setRules(e.target.value)} placeholder={"1. Price above EMA 20\n2. Structure break\n3. Candle confirmation..."} rows={5} style={{ width: "100%", background: C.bg, border: `1px solid ${C.border}`, color: C.text, padding: "9px 12px", borderRadius: 6, fontSize: 12, fontFamily: "IBM Plex Mono, monospace", resize: "vertical" }} /></div>
      ...
      <div style={{ display: "flex", gap: 10 }}><Btn onClick={save} full>Save Pattern</Btn>{p.id && <Btn danger onClick={() => { if (confirm("Delete?")) onDelete(p.id); }}>Delete</Btn>}</div>
    </Modal>
  );
}
...
function TradeViewModal({ trade, acctName, patName, allTrades, patterns, onClose, onEdit }) {
  ...
  const det = (label, val) => (<div style={{ background: C.bg, borderRadius: 6, padding: "10px 12px" }}><div style={{ fontSize: 9, color: C.muted, letterSpacing: 2, marginBottom: 4 }}>{label}</div><div style={{ fontSize: 13, fontWeight: 700 }}>{val || "—"}</div></div>);
  const getAIFeedback = async () => {
    setAiLoading(true); setAiFeedback(null);
    const context = allTrades.filter(t => t.id !== trade.id).slice(-20).map(t => ({ pair: t.pair, type: t.type, pattern: patName(t.patternId), result: t.result, pnl: t.pnl, emotion: t.emotion, day: t.day, session: t.session }));
    const patternRules = patterns.map(p => ({ name: p.name, rules: p.rules, description: p.description }));
    const prompt = `You are an expert forex trading coach. Analyze this trade and respond ONLY with valid JSON:
TRADE: ${JSON.stringify({ pair: trade.pair, type: trade.type, pattern: patName(trade.patternId), result: trade.result, pnl: trade.pnl, emotion: trade.emotion, day: trade.day, session: trade.session, entryNotes: trade.entryNotes || "none", exitNotes: trade.exitNotes || "none", mistakes: trade.mistakes || "none" })}
PATTERN RULES: ${JSON.stringify(patternRules)}
HISTORY: ${JSON.stringify(context)}
JSON: {"summary":"...","whatWentWell":["..."],"whatWentWrong":["..."],"ruleFollowed":true,"ruleAnalysis":"...","emotionImpact":"...","patternFromHistory":"...","oneActionableTip":"..."}`;
    try {
  ...
  return (
    <Modal title={`${trade.pair} · ${(trade.type || "").toUpperCase()}`} onClose={onClose}>
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 18 }}>
        ...
        <div><div style={{ fontFamily: "Bebas Neue, sans-serif", fontSize: 22, letterSpacing: 2 }}>{trade.pair}</div><div style={{ fontSize: 11, color: C.dim }}>{[trade.date, trade.time, trade.day, acctName(trade.accountId)].filter(Boolean).join(" · ")}</div></div>
        <div style={{ marginLeft: "auto", textAlign: "right" }}><div style={{ fontFamily: "Bebas Neue, sans-serif", fontSize: 26, color: (trade.pnl || 0) >= 0 ? C.green : C.red }}>{fmt(trade.pnl)}</div>{trade.riskPct && <div style={{ fontSize: 11, color: C.dim }}>Risk: {trade.riskPct}%</div>}</div>
      </div>
      ...
      {(trade.entryNotes || trade.exitNotes || trade.mistakes || trade.notes) && (
        <div style={{ marginBottom: 14 }}>
          {trade.entryNotes && <div style={{ background: `${C.accent}08`, border: `1px solid ${C.accent}22`, borderRadius: 8, padding: "12px 14px", marginBottom: 8 }}><div style={{ fontSize: 9, color: C.accent, letterSpacing: 2, marginBottom: 5 }}>📝 ENTRY NOTES</div><div style={{ fontSize: 12, lineHeight: 1.7, color: C.text }}>{trade.entryNotes}</div></div>}
          {trade.exitNotes && <div style={{ background: `${C.green}08`, border: `1px solid ${C.green}22`, borderRadius: 8, padding: "12px 14px", marginBottom: 8 }}><div style={{ fontSize: 9, color: C.green, letterSpacing: 2, marginBottom: 5 }}>📝 EXIT NOTES</div><div style={{ fontSize: 12, lineHeight: 1.7, color: C.text }}>{trade.exitNotes}</div></div>}
          {trade.mistakes && <div style={{ background: `${C.red}08`, border: `1px solid ${C.red}22`, borderRadius: 8, padding: "12px 14px", marginBottom: 8 }}><div style={{ fontSize: 9, color: C.red, letterSpacing: 2, marginBottom: 5 }}>⚠️ MISTAKES / ERRORS</div><div style={{ fontSize: 12, lineHeight: 1.7, color: C.text }}>{trade.mistakes}</div></div>}
          {trade.notes && !trade.entryNotes && <div style={{ background: C.bg, borderRadius: 8, padding: "12px 14px", marginBottom: 8 }}><div style={{ fontSize: 9, color: C.muted, letterSpacing: 2, marginBottom: 5 }}>NOTES</div><div style={{ fontSize: 12, lineHeight: 1.7, color: C.text }}>{trade.notes}</div></div>}
        </div>
      )}
      ...
      <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 14, marginBottom: 14 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
          ...
          <Btn onClick={getAIFeedback} disabled={aiLoading} small ghost color={C.accent}>{aiLoading ? "Analyzing..." : aiFeedback ? "Refresh" : "Get AI Feedback"}</Btn>
        </div>
        {aiLoading && (<div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px", background: C.bg, borderRadius: 8 }}><div style={{ width: 20, height: 20, border: `2px solid ${C.border}`, borderTopColor: C.accent, borderRadius: "50%", animation: "spin .8s linear infinite", flexShrink: 0 }} /><div style={{ fontSize: 11, color: C.dim }}>Claude is reviewing your trade and history...</div></div>)}
        {aiFeedback && !aiLoading && (aiFeedback.error ? <div style={{ fontSize: 12, color: C.red, padding: 12, background: C.bg, borderRadius: 8 }}>Could not generate feedback. Try again.</div> : <div>
          <div style={{ background: C.bg, borderRadius: 8, padding: "12px 14px", marginBottom: 10 }}><div style={{ fontSize: 9, color: C.muted, letterSpacing: 2, marginBottom: 6 }}>SUMMARY</div><div style={{ fontSize: 12, lineHeight: 1.7, color: C.text }}>{aiFeedback.summary}</div></div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
            <div style={{ background: "rgba(16,217,138,.06)", border: `1px solid ${C.green}22`, borderRadius: 8, padding: "12px 14px" }}><div style={{ fontSize: 9, color: C.green, letterSpacing: 2, marginBottom: 8 }}>✅ WHAT WENT WELL</div>{(aiFeedback.whatWentWell || []).map((s, i) => <div key={i} style={{ fontSize: 11, color: C.text, lineHeight: 1.7, padding: "3px 0", display: "flex", gap: 6 }}><span style={{ color: C.green }}>▲</span>{s}</div>)}</div>
            <div style={{ background: "rgba(246,59,59,.06)", border: `1px solid ${C.red}22`, borderRadius: 8, padding: "12px 14px" }}><div style={{ fontSize: 9, color: C.red, letterSpacing: 2, marginBottom: 8 }}>⚠️ WHAT WENT WRONG</div>{(aiFeedback.whatWentWrong || []).map((s, i) => <div key={i} style={{ fontSize: 11, color: C.text, lineHeight: 1.7, padding: "3px 0", display: "flex", gap: 6 }}><span style={{ color: C.red }}>▼</span>{s}</div>)}</div>
          </div>
          <div style={{ background: C.bg, borderRadius: 8, padding: "12px 14px", marginBottom: 10 }}><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}><div style={{ fontSize: 9, color: C.muted, letterSpacing: 2 }}>PATTERN RULES</div><span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 3, background: aiFeedback.ruleFollowed ? `${C.green}22` : `${C.red}22`, color: aiFeedback.ruleFollowed ? C.green : C.red }}>{aiFeedback.ruleFollowed ? "✓ Followed" : "✗ Violated"}</span></div><div style={{ fontSize: 12, lineHeight: 1.7, color: C.text }}>{aiFeedback.ruleAnalysis}</div></div>
          <div style={{ background: C.bg, borderRadius: 8, padding: "12px 14px", marginBottom: 10 }}><div style={{ fontSize: 9, color: C.gold, letterSpacing: 2, marginBottom: 6 }}>🧠 EMOTION IMPACT</div><div style={{ fontSize: 12, lineHeight: 1.7, color: C.text }}>{aiFeedback.emotionImpact}</div></div>
          {aiFeedback.patternFromHistory && <div style={{ background: C.bg, borderRadius: 8, padding: "12px 14px", marginBottom: 10 }}><div style={{ fontSize: 9, color: C.muted, letterSpacing: 2, marginBottom: 6 }}>📊 PATTERN FROM YOUR HISTORY</div><div style={{ fontSize: 12, lineHeight: 1.7, color: C.text }}>{aiFeedback.patternFromHistory}</div></div>}
          <div style={{ background: `${C.accent}08`, border: `1px solid ${C.accent}33`, borderRadius: 8, padding: "12px 14px" }}><div style={{ fontSize: 9, color: C.accent, letterSpacing: 2, marginBottom: 6 }}>⊕ #1 TIP FOR NEXT TIME</div><div style={{ fontSize: 13, fontWeight: 700, color: C.accent, lineHeight: 1.6 }}>{aiFeedback.oneActionableTip}</div></div>
        </div>)}
        {!aiFeedback && !aiLoading && <div style={{ fontSize: 11, color: C.muted, lineHeight: 1.7, padding: "10px 0" }}>Click "Get AI Feedback" to receive a personalized analysis of this trade.</div>}
      </div>
...
function PatternViewModal({ pattern, trades, onClose, onEdit }) {
  ...
  return (
    <Modal title={pattern.name} onClose={onClose}>
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>{pattern.timeframe && <Tag color={C.accent}>{pattern.timeframe}</Tag>}{pattern.session && <Tag>{pattern.session}</Tag>}</div>
      ...
      {pattern.confirmations && <div style={{ marginBottom: 14 }}><div style={{ fontSize: 9, color: C.muted, letterSpacing: 2, marginBottom: 5 }}>CONFIRMATIONS</div><div style={{ fontSize: 12 }}>{pattern.confirmations}</div></div>}
      {pt.length > 0 && <div style={{ background: C.bg, borderRadius: 8, padding: "12px 14px", marginBottom: 14 }}><div style={{ fontSize: 9, color: C.muted, letterSpacing: 2, marginBottom: 6 }}>PERFORMANCE</div><div style={{ fontFamily: "Bebas Neue, sans-serif", fontSize: 22, color: pnl >= 0 ? C.green : C.red }}>{fmt(pnl)}</div><div style={{ fontSize: 11, color: C.dim, marginTop: 3 }}>{wins}/{pt.length} wins · {Math.round(wins / pt.length * 100)}% WR</div></div>}
